...
class StopCountdown : UnconfinedTest() {
  @Test
  ...
  @Test
  fun `set changed`() = runTest(UnconfinedTestDispatcher()) {
    val data = ExerciseWithSets(
      exercise = Exercise(routineId = 0L), sets = listOf(
        ExerciseSet(exerciseId = 0L, value = 3, valueType = ExerciseSet.ValueType.COUNTDOWN),
      )
    ...
    viewmodel.stopCountdown()
    TestCase.assertEquals(1, viewmodel.uiState.value?.currentSet?.value?.index)
  }
}